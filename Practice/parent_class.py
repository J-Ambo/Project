...
class Parent:
    ...
    def calculate_distance_to_birds(self, other_bird):
        ...
        return np.linalg.norm(self.pos - other_bird.pos)

    def point_is_out_of_bounds(self, coordinate, environment):
        if coordinate >= environment.size:
            return True
        elif coordinate <= 0:
            return True
        else:
    ...
    def apply_boudary_condition(self, coordinate, environment):
        if coordinate >= environment.size:
            new_coordinate = environment.size - 0.01*environment.size
            return new_coordinate
        elif coordinate <= 0:
            new_coordinate = 0 + 0.01*environment.size
            return new_coordinate
