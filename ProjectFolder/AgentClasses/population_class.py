...
class Population(Parent):
    ...
    def __init__(self, population_size, environment, predator):
     ...
     ###   theta = np.random.uniform(0, np.pi, self.population_size)
        ...
        z = np.random.uniform(2, self.depth*0.9, self.population_size)  #r * np.cos(theta)
        x = r * np.cos(phi)        #r * np.cos(phi)*np.sin(theta)
        y = r * np.sin(phi)        #r * np.sin(phi)*np.sin(theta)
    ...
    def calculate_wall_vectors(self, focus_position, focus_direction):#environment, distances_from_origin):
      ...
      ###  all_wall_vectors = np.where(distances_from_origin[:, np.newaxis] >= environment.radius, -self.population_positions[:-1], all_wall_vectors)
       ...
       # v_w = np.array([0,0,-focus_position[-1]])/focus_position[-1]
        v_w = focus_direction * np.array([1, 1, -1])

      #  print(focus_position, focus_position[-1], focus_direction, v_w)
        return v_w
    ...
    def calculate_all_vectors(self, neighbours, distances, predator):
        ...
        for index in range(len(self.population_positions[:-1])):
            ...
            if np.all(self.all_repulsion_vectors[index] != 0):
                continue
      ...
      ##  print(self.population_positions[0], self.population_directions[0], self.all_wall_vectors[0], self.all_repulsion_vectors[0], self.all_alignment_vectors[0], self.all_attraction_vectors[0],neighbours[0].size)
    ...
    ###    print(np.linalg.norm(self.all_repulsion_vectors,axis=1), np.linalg.norm(self.all_alignment_vectors,axis=1), np.linalg.norm(self.all_attraction_vectors,axis=1))
     ...
     #   print('Sum', sum_of_social_vectors)
        ...
        #print(all_escape_coefficients, all_social_coefficients)
    ...
    def calculate_new_directions(self, tree, environment, predator):
      ...
      ###  boundary_mask = distances_from_origin >= environment.radius
       # print(self.population_positions, self.population_positions[:-1,-1])
        boundary_mask = (self.population_positions[:-1,-1] < 0) | (self.population_positions[:-1,-1] > self.depth)
      #  print(boundary_mask[0])

      ###  print(self.population_positions[:-1,-1], oundary_mask)
    ...
    def update_positions(self, tree, environment, predator):
      ...
      ##  print(self.population_positions[0], self.population_directions[0])

    def remove_outliers(self):
        lof = LocalOutlierFactor(n_neighbors=3, algorithm='kd_tree', contamination='auto')
