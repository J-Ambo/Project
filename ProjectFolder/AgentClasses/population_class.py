...
class Population(Parent):
    ...
    def __init__(self, population_size, environment, predator):
        ...
        r = np.random.uniform(0, 100, self.population_size)
        phi = np.random.uniform(0, 2*np.pi, self.population_size)
        theta = np.random.uniform(0, np.pi, self.population_size)

        z = r * np.cos(theta)   #np.random.uniform(2, self.depth*0.9, self.population_size)  #
        x = r * np.cos(phi)*np.sin(theta)   #r * np.cos(phi)        #
        y = r * np.sin(phi)*np.sin(theta)   #r * np.sin(phi)        #
    ...
    def calculate_wall_vectors(self, focus_position, focus_direction):
    ...
    ###        return v_r
    ###    else:
    ###        return v_r/len(neighbour_positions)
        return v_r
    ...
    ###        return v_al
    ###    else:
    ###       return v_al/len(neighbour_directions)
        return v_al
    ...
    ###        return v_at
    ###    else:
    ###        return v_at/len(neighbour_positions)
        return v_at
    ...
    def calculate_all_vectors(self, neighbours, distances, predator):
        ...
        ###    if (index_position[-1] < 0) | (index_position[-1] > self.depth):     # Before looking at neighbours check if the current fish is out of bounds
         ###       self.all_wall_vectors[index] = self.calculate_wall_vectors(index_position, self.population_directions[index])
               # if index_position[-1] < 0:
               #     self.population_positions[index][-1] = 0
                #elif index_position[-1] > self.depth:
                #    self.population_positions[index][-1] = self.depth
        ###        continue
            ...
            if index_neighbours[repulsion_zone_mask].size >= 1:     
        ...
        ###    print(self.all_repulsion_vectors[index], neighbours[index][(index_social_neighbours_distances > 0) & (index_social_neighbours_distances <= Parent.rr)], self.all_attraction_vectors[index], neighbours[index][(index_social_neighbours_distances > Parent.ral) & (index_social_neighbours_distances <= Parent.rat)] )
    ...
    ###    print(np.linalg.norm(self.all_repulsion_vectors,axis=1), np.linalg.norm(self.all_alignment_vectors,axis=1), np.linalg.norm(self.all_attraction_vectors,axis=1))
 ...
 ####       print('sum soc norm',sum_of_social_vectors[0])
 ...
 ####       print('sum all norm',sum_of_all_vectors[0])
   ...
   ####     print(self.all_repulsion_vectors[0], self.all_alignment_vectors[0], self.all_attraction_vectors[0], target_directions[0], self.population_directions[0],
    ####           angles_to_target_directions[0], maximal_directions[0])
...
#        boundary_mask = (self.population_positions[:-1,-1] < 0) | (self.population_positions[:-1,-1] > self.depth)
...
##            self.population_directions[:-1][boundary_mask] = self.all_wall_vectors[boundary_mask]
    ...
    def remove_outliers(self):
        lof = LocalOutlierFactor(n_neighbors=30, algorithm='kd_tree', contamination='auto')
  ...
  #  def calculate_average_inlier_position(self):
    ...
    ##    print(school_size, np.argmax(self.population_densities), np.argmax(self.neighbour_lens))
