...
for i in range(2):
    ...
    for j in range(5):
        ax1.errorbar(X1, data[i][j], yerr=errs[i][j], capsize=2, color=colours[i][j], label=f'${lb[j]} \degree s^{{-1}}$')
    ax1.invert_xaxis()
    ax1.legend(frameon=False, fontsize=14, loc='upper right', labelspacing=0.3, handlelength=1.8, handletextpad=0.5)
